...
export const ImageCanvas: React.FC<ImageCanvasProps> = ({ promptItem, user, onUpdate, readOnly = false }) => {
  ...
  // Sharing State
  const [shareVersion, setShareVersion] = useState<PromptVersion | null>(null);
  ...
  return (
    <div className="relative w-full h-full flex flex-col overflow-hidden bg-canvas">
      ...
      <div
        className={`absolute top-0 left-0 right-0 z-50 flex flex-col items-center transition-transform duration-500 ease-in-out pointer-events-none ${isInputCollapsed ? '-translate-y-[calc(100%-40px)]' : 'translate-y-0'
          }`}
      >
        {/* Spacer */}
                  ...
                  <CreditCheck serviceType="image_generation" onCreditCheck={setCreditCheckResult}>
                    {({ sufficient, cost, checking, error, checkResult }) => (
                      <button
                        ...
                        className={`px-6 py-2 rounded-lg text-white text-sm font-medium flex items-center gap-2 shadow-lg transition-colors relative ${sufficient
                      ...
                      {!isVideo && version.imageUrl && (
                        <>
                          ...
                          <button
                            ...
                            onMouseDown={(e) => e.stopPropagation()}
                            onMouseUp={(e) => e.stopPropagation()}
                            className="p-1.5 hover:bg-indigo-500/20 rounded text-indigo-400 hover:text-indigo-300"
                            title="分享卡片 (装逼利器)"
                          >
                            <Icons.Play className="w-3 h-3 -rotate-45" />
                          </button>
                        </>
                      )}

        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-4 rounded-2xl ${msg.role === 'user'
              ...
              <div className={`markdown-body !bg-transparent !text-inherit text-sm leading-relaxed ${msg.role === 'user' ? '!text-white' : ''}`}>
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  rehypePlugins={[rehypeRaw, rehypeHighlight]}
                  components={{
                    pre: ({ node, ...props }) => <pre className="!bg-black/30 !rounded-lg !p-4 !my-2 overflow-x-auto" {...props} />,
                    code: ({ node, inline, ...props }: any) =>
                      inline
                        ? <code className="bg-black/20 px-1 rounded text-accent" {...props} />
                        : <code {...props} />,
                    ...
                    blockquote: ({ node, ...props }) => <blockquote className="border-l-4 border-slate-500 pl-4 italic my-2 opacity-80" {...props} />,
                  }}
                >
                  {msg.text}
                </ReactMarkdown>
              ...
              <div className="flex items-center justify-end gap-2 mt-2">
                <button
                  ...
                  className={`flex items-center gap-1 p-1 rounded transition-all duration-200 ${copiedIndex === idx
                ...
                >
                  {copiedIndex === idx ? (
                    <>
                      ...
                      <span className="text-[10px] font-medium">已复制</span>
                    </>
                  ) : (
              ...
              <CreditCheck serviceType="text_chat" onCreditCheck={setCreditCheckResult}>
                {({ sufficient, cost, checking, error, checkResult }) => (
                  <button
                    ...
                    className={`w-14 h-10 mb-1 flex items-center justify-center rounded-xl text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed self-end relative ${sufficient
